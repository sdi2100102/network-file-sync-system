...
void client_run(int manager_socket);
...
void client_run(int manager_socket)
{
    ...
    if (strcmp(command_type, "list") == 0)
    {
        ...
        if (source_dir == NULL)
            return; // todo replace with exit thread
    ...
    return; // todo replace with thread exit
}
...
void command_push(char *path_name, int chunk_size, char *data, int manager_socket)
{
    ...
    default:
        ...
        while (chunk_size > 0)
        {
            int bytes_read = read(manager_socket, buffer, chunk_size);
            if (bytes_read == -1)
...
void send_string(int manager_socket, char string[])
{
    if (write(manager_socket, string, strlen(string)) == -1)
